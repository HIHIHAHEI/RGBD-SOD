...
from model.Resnet101 import resnet101_dy
from model.Resnet101_DB import resnet101_DB
from model.resnet2d import ResNet50
...
class Reduction3D(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.reduce = nn.Sequential(
            BasicConv2d(in_channel, out_channel, kernel_size=[1, 1]),
...
class ThreeDDecoder(nn.Module):
    def __init__(self, channel):
        ...
        self.conv_upsample1 = BasicConv2d(channel, channel, [3, 3], padding=1)
        self.conv_upsample2 = BasicConv2d(channel, channel, [ 3, 3], padding=1)
        self.conv_upsample3 = BasicConv2d(channel, channel, [ 3, 3], padding=1)
        self.conv_upsample4 = BasicConv2d(channel, channel, [ 3, 3], padding=1)

        self.conv_downsample2_1 = BasicConv2d(channel, channel, [ 3, 3], stride=[ 2, 2], padding=[ 1, 1])
        self.conv_downsample1_1 = BasicConv2d(channel, channel, [ 3, 3], stride=[ 2, 2], padding=[ 1, 1])
        self.conv_downsample1_2 = BasicConv2d(channel, channel, [ 3, 3], stride=[ 2, 2], padding=[ 1, 1])
        self.conv_downsample0_1 = BasicConv2d(channel, channel, [ 3, 3], stride=[ 2, 2], padding=[ 1, 1])
        self.conv_downsample0_2 = BasicConv2d(channel, channel, [ 3, 3], stride=[ 2, 2], padding=[ 1, 1])
        self.conv_downsample0_3 = BasicConv2d(channel, channel, [ 3, 3], stride=[ 2, 2], padding=[ 1, 1])

        self.conv_cat1 = nn.Sequential(
            BasicConv2d(channel*2, channel, [ 3, 3], padding=1),
            BasicConv2d(channel, channel*2, [ 1, 1])
        )
        ...
        self.conv_cat3 = nn.Sequential(
            BasicConv2d(channel*4, channel, [3, 3], padding=1),
            BasicConv2d(channel, channel*4, [ 1, 1])
        )
        self.conv_cat4 = nn.Sequential(
            BasicConv2d(channel*5, channel, [ 3, 3], padding=1),
            BasicConv2d(channel, channel*5, [ 1, 1])
        )
        ...
        self.downt1 = BasicConv2d(channel*5, channel, [ 1, 1])
        self.downt2 = BasicConv2d(channel*4, channel, [ 1, 1])
        self.downt3 = BasicConv2d(channel*3, channel, [ 1, 1])
        self.downt4 = BasicConv2d(channel*2, channel, [ 1, 1])
        ...
        self.se3 = SELayer(channel*5 )
        self.se2 = SELayer(channel*4 )
        self.se1 = SELayer(channel *3)
        self.se0 = SELayer(channel *2)
    ...
    def forward(self, x0, x1, x2, x3, x4):
        ...
        # 3,4
        B, C4, H4, W4 = x4.size()
        x4_flatten_up = self.upsample(x4)
        x4_upconv = self.conv_upsample4(x4_flatten_up)
        residual3 = torch.cat((x4_upconv, x3, x2_downconv1, x1_downconv2, x0_downconv3), 1)
        x3_ = self.conv_cat4(residual3)
        x3_flatten = self.se3(x3_)
        x3 = residual3 + x3_flatten
        x3 = self.downt1(x3)

        # 2,3
        B, C3,  H3, W3 = x3.size()
        x3_flatten_up = self.upsample(x3)
        x3_upconv = self.conv_upsample3(x3_flatten_up)
        residual2 = torch.cat((x3_upconv, x2, x1_downconv1, x0_downconv2), 1)
        x2_ = self.conv_cat3(residual2)
        x2_flatten = self.se2(x2_)
        x2 = residual2 + x2_flatten
        x2 = self.downt2(x2)

        # 1,2
        B, C2, H2, W2 = x2.size()
        x2_flatten_up = self.upsample(x2)
        x2_upconv = self.conv_upsample2(x2_flatten_up)
        residual1 = torch.cat((x2_upconv, x1, x0_downconv1), 1)
        x1_ = self.conv_cat2(residual1)  # 64
        x1_flatten = self.se1(x1_)
        x1 = residual1 + x1_flatten
        x1 = self.downt3(x1)

        # 0,1
        B, C1, H1, W1 = x1.size()
        x1_flatten_up = self.upsample(x1)
        x1_upconv = self.conv_upsample1(x1_flatten_up)
        residual0 = torch.cat((x0, x1_upconv), 1)
        x0_ = self.conv_cat1(residual0)
        x0_flatten = self.se0(x0_)
        x0 = residual0 + x0_flatten
        x0 = self.downt4(x0)
        # x = x0.squeeze(2)
...
class DownBlock(torch.nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0):
        ...
        self.down_conv1 = BasicConv2d(in_channels, out_channels, kernel_size, stride, padding)
        self.down_conv2 = DeconvBlock(in_channels, out_channels, kernel_size, stride, padding)
        self.down_conv3 = BasicConv2d(in_channels, out_channels, kernel_size, stride, padding)
...
class RD3D(nn.Module):
    def __init__(self, channel, pretrain):
